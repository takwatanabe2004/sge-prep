function [w,output]=tak_GN_group_logistic_FISTA_linesearch(X,y,lam,gam,options,C,nvol,wtrue)
% [w,output]=tak_GN_group_logistic_FISTA_linesearch(X,y,lam,gam,options,C,nvol,wtrue)
% (08/18/2015)
%==============================================================================%
% - FISTA GraphNet logistic: line search version
%  (function value always computed since it's needed for Armjio check anyways)
%------------------------------------------------------------------------------%
%    sum(log(1+exp(-YWx))  + lam * ||w||_2,1 + gam/2 * ||C*w||^2
%==============================================================================%
...
%| proximal operator (group L21 penalty)
prox_op = @(w,tau) vec( tsoftvec( reshape(w,[p/nvol,nvol]), tau)  );
...
% loss = sum(logis(yXw));
%=========================================================================%
L21 = @(w) tak_norm_L21( reshape(w, [p/nvol, nvol]) ); % L21 norm

logis = @(t) log(1 + exp(-t));
fval = @(w) sum(logis(YX*w)) + lam*L21(w) + gam/2*norm(C*w)^2;
...
%     k
    ...
    %| proximal operator (vector threshold in this case)
    w_test = prox_op( v - tau*gradv, lam*tau);
